...
- **File storage**: `ingestUrl` captures and stores the original fetched document plus a cleaned HTML snapshot (`sources.original_file_path` / `snapshot_file_path`) via a pluggable `StorageDriver` (`src/lib/storage.ts`). `local` (disk) and `appwrite` (Appwrite Storage, see `docs/deployment.md`) both work end to end; `S3_*` env vars exist in the schema but have no driver implementation yet.
